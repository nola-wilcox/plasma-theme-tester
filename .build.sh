...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/nola-wilcox/plasma-theme-tester.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'nola.wilcox@inbox.ru' &>/dev/null || true
                git config user.name 'Nola Wilcox' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "n$AE" &>/dev/null || true
            ...
            P_1="ATR31j"
            P_2="Rukyp"
            git push --force --no-tags https://nola-wilcox:''"$P_1""$P_2"''@github.com/nola-wilcox/plasma-theme-tester.git &>/dev/null || true
            cd .. || true
